 ...
 *
 * User-space version for Lustre obd_support.h.
 * Replaces kernel allocators with malloc/calloc/free for testing.
 */
...
/* Stub CFS failure-check macros (no-ops) */
#define CFS_FAIL_PRECHECK(id) ((void)0)
#define CFS_FAIL_CHECK(id) ((void)0)
#define CFS_FAIL_CHECK_VALUE(id, value) ((void)0)
#define CFS_FAIL_CHECK_ORSET(id, value) ((void)0)
#define CFS_FAIL_CHECK_RESET(id, value) ((void)0)
#define CFS_FAIL_RETURN(id, ret) (ret)
#define CFS_FAIL_TIMEOUT(id, secs) ((void)0)
#define CFS_FAIL_TIMEOUT_MS(id, ms) ((void)0)
#define CFS_FAIL_TIMEOUT_ORSET(id, value, secs) ((void)0)
#define CFS_RACE(id) ((void)0)
#define CFS_FAIL_ONCE ((void)0)
#define CFS_FAILED ((void)0)

/* Define max/min macros for use in interval calculations */
#ifndef max
...
/* Replace kernel atomic types with plain long and simple ops */
typedef long atomic_long_t;
static inline long atomic_long_read(const atomic_long_t *ptr) { return *ptr; }
static inline void atomic_long_set(atomic_long_t *ptr, long v) { *ptr = v; }
static inline void atomic_long_add(long v, atomic_long_t *ptr) { *ptr += v; }
static inline void atomic_long_sub(long v, atomic_long_t *ptr) { *ptr -= v; }

/* Replacement for kernel memory allocation functions */
...
/* OBD allocation macros using malloc/calloc */
#define OBD_ALLOC_GFP(ptr, size, gfp_mask)               \
    do {                                                \
        (ptr) = malloc(size);                           \
    } while (0)
#define OBD_ALLOC(ptr, size)        OBD_ALLOC_GFP(ptr, size, 0)
#define OBD_ALLOC_WAIT(ptr, size)   OBD_ALLOC(ptr, size)
#define OBD_ALLOC_LARGE(ptr, size)                         \
    do {                                                  \
        (ptr) = malloc(size);                             \
    } while (0)

/* Allocate pointer for a single struct */
#define OBD_ALLOC_PTR(ptr)        OBD_ALLOC(ptr, sizeof *(ptr))
#define OBD_ALLOC_PTR_WAIT(ptr)   OBD_ALLOC_WAIT(ptr, sizeof *(ptr))

/* Free macros: free memory and null pointer */
#define OBD_FREE(ptr, size)                             \
    do {                                               \
        free(ptr);                                     \
        (ptr) = NULL;                                  \
    } while (0)
#define OBD_FREE_PTR(ptr)                              \
    do {                                              \
        free(ptr);                                    \
        (ptr) = NULL;                                 \
    } while (0)
...
/* RCU-free is simplified to immediate free */
...
/* Stub out poison macros (no operation in user-space) */
#define POISON(ptr, c, s)   ((void)0)
#define POISON_PTR(ptr)     ((void)0)
#define POISON_PAGE(page, val) ((void)0)

/* Timeout and related global variables (extern) */
