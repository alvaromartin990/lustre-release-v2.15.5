...
analyze_logs() {
    ...
    for stage in 1 2 3 4 5; do
        ...
        if [[ $stage_count -gt 0 ]]; then
            grep "Stage $stage" "$log_file" | head -5
        else
    ...
    # Show actual execution order per file creation
    echo "Execution Order Analysis (per file creation):"
    echo "Showing order of stage execution for each file:"
    local file_num=1
    grep -E "(MDT_DEBUG.*OPEN|Stage [1-5])" "$log_file" | while read -r line; do
    ...
    if command -v awk >/dev/null 2>&1; then
        ...
        for stage in 1 2 3 4 5; do
            ...
            if [[ $stage_count -gt 0 ]]; then
                ...
                grep "Stage $stage" "$log_file" | head -3 | sed 's/^/    /'
                echo ""
            else
        ...
        for op in "osd_trans_exec_op" "osd_create_type_f" "unlock_new_inode" "osd_attr_init" "osd_object_init0" "osd_trans_exec_check" "__osd_create total" "osd_ea_fid_set" "__osd_oi_insert" "osd_idc_find_and_init" "Total osd_create"; do
            local avg=$(grep "$op took" "$log_file" | awk '{print $(NF-1)}' | awk '{sum+=$1; count++} END {if(count>0) printf "%.1f", sum/count; else print "0"}')
            printf "  %-35s: %s µs\n" "$op" "$avg"
        done
        echo ""
