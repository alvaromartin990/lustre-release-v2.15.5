...
capture_kernel_logs() {
    ...
    # Try multiple methods to get kernel logs
    {
        ...
        echo "=== Looking for MDT_TIMING, MDD_TIMING, OSD_TIMING and REINT_OPEN messages ==="
        echo ""
        
        # Method 1: dmesg (most common)
        if command -v dmesg >/dev/null 2>&1; then
            echo "=== DMESG OUTPUT ==="
            dmesg | grep -E "(MDT_TIMING|MDD_TIMING|OSD_TIMING|REINT_OPEN|$KERNEL_LOG_MARK)" | tail -100
            echo ""
        fi
        ...
        if command -v journalctl >/dev/null 2>&1; then
            ...
            journalctl --dmesg --no-pager | grep -E "(MDT_TIMING|MDD_TIMING|OSD_TIMING|REINT_OPEN|$KERNEL_LOG_MARK)" | tail -100
            echo ""
        fi
        ...
        for log_file in /var/log/messages /var/log/kern.log; do
            if [[ -r "$log_file" ]]; then
                ...
                grep -E "(MDT_TIMING|MDD_TIMING|OSD_TIMING|REINT_OPEN|$KERNEL_LOG_MARK)" "$log_file" | tail -50
                echo ""
            fi
        done
...
# Main execution
main() {
    print_info "Lustre OPEN File Performance Test"
    print_info "================================="
